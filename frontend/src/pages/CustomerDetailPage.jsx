  ...
  if (loading) {
    return (
      <div className="p-8 max-w-5xl mx-auto space-y-6">
        <div className="h-8 w-48 bg-white dark:bg-slate-800 rounded-xl animate-pulse" />
        <div className="h-48 bg-white dark:bg-slate-800 rounded-xl animate-pulse" />
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      ...
      <button
        ...
        className="flex items-center gap-1 text-sm font-bold text-sky-600 hover:text-sky-700 dark:text-sky-300 transition-colors"
      >
      ...
      {/* Customer Info Card */}
      <div className="bg-white dark:bg-slate-800 rounded-xl p-8 shadow-sm dark:shadow-slate-950/20 border border-slate-100 dark:border-slate-700">
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-extrabold font-headline tracking-tight text-on-surface">{customer.name}</h2>
          ...
          <button
            ...
            className={`flex items-center gap-1 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
              editing ? 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300' : 'bg-sky-50 dark:bg-sky-500/10 text-sky-600 hover:bg-sky-100 dark:hover:bg-sky-500/20'
            }`}
          >
        ...
        {editing ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div>
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Phone</p>
              <p className="text-sm font-semibold text-on-surface">{customer.phone || '—'}</p>
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1">Email</p>
      ...
      {/* Service History */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-slate-950/20 border border-slate-100 dark:border-slate-700 overflow-hidden">
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900 border-b border-slate-100 dark:border-slate-700">
          <h3 className="text-sm font-bold text-on-surface uppercase tracking-widest flex items-center gap-2">
            ...
            Service History
            <span className="ml-auto text-xs font-semibold text-slate-500 dark:text-slate-400">{records.length} record{records.length !== 1 ? 's' : ''}</span>
          </h3>
        </div>
        {records.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[10px] uppercase tracking-[0.15em] font-extrabold text-slate-500 dark:text-slate-400 bg-slate-50/50">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                {records.map((r) => (
                  <tr key={r.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/50 dark:hover:bg-slate-700/50 transition-colors">
                    <td className="px-6 py-3 text-sm font-bold text-on-surface">{r.vehicle_name}</td>
                    <td className="px-6 py-3">
                      <span className="font-mono text-xs font-semibold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded">{r.license_plate}</span>
                    </td>
                    <td className="px-6 py-3">
                    ...
                    <td className="px-6 py-3 text-xs font-semibold text-slate-600 dark:text-slate-300">{r.created_at?.split(' ')[0] || '—'}</td>
