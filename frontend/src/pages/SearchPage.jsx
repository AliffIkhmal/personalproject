  ...
  return (
    <section className="p-8 max-w-7xl mx-auto">
      ...
      <div className="mt-12 mb-16 max-w-3xl mx-auto text-center">
        ...
        <form onSubmit={handleSearch} className="relative group">
          ...
          <div className="absolute inset-y-0 right-4 flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${showFilters || hasActiveFilters ? 'bg-sky-100 dark:bg-sky-500/20 text-sky-600 dark:text-sky-400' : 'text-slate-400 dark:text-slate-500 hover:text-sky-500 hover:bg-sky-50 dark:hover:bg-sky-500/10'}`}
        ...
        {/* Filter Panel */}
        {showFilters && (
          <div className="mt-4 p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm dark:shadow-slate-950/20 border border-slate-100 dark:border-slate-700 text-left">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest px-1">From Date</label>
                <input
                  type="date"
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest px-1">To Date</label>
                <input
                  type="date"
            ...
            {hasActiveFilters && (
              <div className="mt-3 flex justify-end">
                <button
                  onClick={clearAll}
                  className="text-xs font-bold text-sky-600 hover:text-sky-700 dark:text-sky-300 transition-colors"
                >
                  Clear All Filters
                </button>
              </div>
      ...
      {results !== null && !loading && (
        results.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.map((r) => (
              <div key={r.id} className="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm dark:shadow-slate-950/20 hover:shadow-xl transition-all duration-300 relative group overflow-hidden border border-transparent hover:border-sky-100 dark:hover:border-sky-500/20">
                <div className={`absolute left-0 top-0 bottom-0 w-1 ${
                  r.status === 'in_progress' ? 'bg-amber-500' : r.status === 'completed' ? 'bg-emerald-500' : 'bg-sky-500'
                }`} />
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="font-headline font-bold text-lg text-on-surface">{r.vehicle_name}</h3>
                    <code className="text-sm font-mono text-sky-600 font-bold bg-sky-50 dark:bg-sky-500/10 px-2 py-0.5 rounded">{r.license_plate}</code>
                  </div>
                ...
                <div className="space-y-3 mb-6">
                  ...
                  {isAdmin && r.technician_name && (
                    <div className="flex items-center gap-3">
                      <span className="material-symbols-outlined text-outline text-lg">badge</span>
                      <span className="text-sm font-medium text-on-surface-variant">{r.technician_name}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-3">
